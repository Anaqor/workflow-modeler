...
export default class PlanQKMenuProvider {
  ...
  getPopupMenuEntries(element) {
    ...
    return function (entries) {
      ...
      if (
        !(element.type.startsWith("bpmn") || element.type.startsWith("planqk"))
      ) {
        return entries;
      }
      ...
      if (is(element, consts.PLANQK_DATA_POOL)) {
        const dataPoolEntries = self.createDataPoolEntries(
          element,
          self.dataPools
        );
      ...
      if (is(element, "bpmn:DataObjectReference")) {
        let filteredOptions = filter(
          planqkReplaceOptions.DATA_STORE,
          isDifferentType(element)
        );
        const planqkEntries = createMenuEntries(
          element,
          filteredOptions,
          self.translate,
          self.replaceElement
  ...
  createDataPoolEntries(element, dataPools) {
    ...
    if (element.businessObject.name) {
      dataPoolEntries["replace-with-generic-data-pool"] =
        this.createNewDataPoolEntry(element, {
    ...
    for (let dataPool of dataPools) {
      if (element.businessObject.name !== dataPool.name) {
        dataPoolEntries["replace-with-" + dataPool.id + " (2)"] =
          this.createNewDataPoolEntry(element, dataPool);
      }
    }
